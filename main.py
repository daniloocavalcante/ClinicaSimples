...
tv_med.column("ID", width=20)
tv_med.column("Nome", width=100)
tv_med.column("Especialidade", width=100)

tv_med.pack(fill="both", expand=True, pady=10, padx=20)
...
def mascara_data_paciente(event):
    texto = p_nasc.get().replace("/", "")    
    if len(texto) > 2:
        texto = texto[:2] + "/" + texto[2:]
    if len(texto) > 5:
        texto = texto[:5] + "/" + texto[5:9]
    p_nasc.delete(0, tk.END)
    p_nasc.insert(0, texto)

p_nome = tk.Entry(frm_pac, width=40)
p_nasc = tk.Entry(frm_pac, width=40)
p_nasc.bind("<KeyRelease>", mascara_data_paciente)
...
tv_pac.pack(fill="both", expand=True, pady=10, padx=20)

tv_pac.column("ID", width=20)
tv_pac.column("Nome", width=100)
tv_pac.column("Nascimento", width=100)
...
def mascara_hora(event):
    texto = c_hora.get().replace(":", "")
    if len(texto) > 2:
        texto = texto[:2] + ":" + texto[2:4]
    c_hora.delete(0, tk.END)
    c_hora.insert(0, texto)

def mascara_data(event):
    texto = c_data.get().replace("/", "")    
    if len(texto) > 2:
        texto = texto[:2] + "/" + texto[2:]
    if len(texto) > 5:
        texto = texto[:5] + "/" + texto[5:9]
    c_data.delete(0, tk.END)
    c_data.insert(0, texto)
...
c_data.bind("<KeyRelease>", mascara_data)
c_hora = tk.Entry(frm_con, width=20)
c_hora.bind("<KeyRelease>", mascara_hora)
...
tv_con.pack(fill="both", expand=True, pady=10,padx=20)

tv_con.column("ID", width=30)
tv_con.column("Data", width=60)
tv_con.column("Hora", width=40)
tv_con.column("Médico", width=100)
tv_con.column("Paciente", width=100)
